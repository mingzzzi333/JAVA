...
public class D0_CardDivide {

	public ArrayList<D0_Player> players;// 플레이어들을 담는 리스트
	public final int cardNum=2;//플레이어에게 나눠줄 카드 장수 
	public D0_CardDivide() {
		players=new ArrayList<D0_Player>();
	...
	public void numPlayer(){
		...
		for (int i = 0; i < pNum; i++) {
			D0_Player p=new D0_Player();
			players.add(p);
		}
	}
	//카드를 플레이어들에게 나눠주는 메서드
	public void cardDivide(int n){
		D0_CardDeck deck=new D0_CardDeck();
		ArrayList<D0_Card> cardCase=deck.getCard();
		for (int i = 0; i < n; i++) {
	...
	//두 장의 카드의 점수를 환산한다.
	public int check(D0_Player pUser){
